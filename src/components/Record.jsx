...
const Record = ({ data }) => {
  const [liked, setLiked] = useState(
    localStorage.getItem("favorites", data.id)?.includes(data.id)
      ? true
      : false,
  );
  const changeFavorite = () => {
    let currentData = localStorage.getItem("favorites")
    ...
    <div className="h-full w-3/12 max-lg:w-4/12 max-md:w-6/12 max-sm:w-screen relative p-4">
      ...
      <div className="absolute top-8 bottom-0 right-8 w-8 h-8 rounded-full bg-white flex justify-center items-center">
        {liked ? (
          ...
          <FaRegHeart
            className={"text-black opacity-60 w-full h-full p-1.5"}
            onClick={() => changeFavorite()}
          />
        )}
      </div>
